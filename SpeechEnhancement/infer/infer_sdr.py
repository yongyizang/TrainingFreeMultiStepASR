...
os.makedirs("/root/autodl-tmp/inferred_output_vctkdemucs_sdr/step_0", exist_ok=True)
...
for noisy_file in tqdm((noisy_files), total=len(noisy_files)):
    ...
    save_audio(f"/root/autodl-tmp/inferred_output_vctkdemucs_sdr/step_0/{filename}", inferred[0], fs)
...
for i in tqdm(range(inference_steps)):
    ...
    prev_step_dir = f"/root/autodl-tmp/inferred_output_vctkdemucs_sdr/step_{i}"
    current_step_dir = f"/root/autodl-tmp/inferred_output_vctkdemucs_sdr/step_{i + 1}"
    ...
    candidate_ratios = [start_ratio + j * ratio_step for j in range(candidate_each_step)]
    ...
    for target_file, clean_file, noisy_file in tqdm(zip(target_files, clean_files, noisy_files), total=len(target_files)):
        ...
        sdrs = [sdr(target_audio, inferred) for inferred in candidate_inferred]
